...
<div class="space-y-8">
	<h1 class="text-2xl font-bold">Wortgruppen (Decks)</h1>
	...
	<section class="rounded-xl border border-gray-200 bg-white p-5">
		...
		<form method="POST" action="?/create" use:enhance class="grid gap-3 sm:grid-cols-4">
			<label class="block space-y-1 sm:col-span-2">
				...
				<input name="name" required placeholder="z. B. Englisch – Tiere" class="w-full rounded border border-gray-300 px-3 py-2" />
			</label>
			<label class="block space-y-1">
				<span class="text-sm font-medium">von (Sprache)</span>
				<input name="language_from" required value="de" class="w-full rounded border border-gray-300 px-3 py-2" />
			</label>
			<label class="block space-y-1">
				<span class="text-sm font-medium">nach (Sprache)</span>
				<input name="language_to" required value="en" class="w-full rounded border border-gray-300 px-3 py-2" />
			</label>
					...
					<li class="space-y-2 py-3">
						<div class="flex flex-wrap items-center justify-between gap-2">
							...
							<div class="flex items-center gap-2">
								<a href="/decks/{deck.id}" class="rounded border px-2 py-1 text-sm hover:bg-gray-50">Wörter</a>
								<button type="button" onclick={() => (editId = editId === deck.id ? null : deck.id)} class="rounded border px-2 py-1 text-sm hover:bg-gray-50">Bearbeiten</button>
						...
						{#if editId === deck.id}
							<form method="POST" action="?/update" use:enhance={() => async ({ update }) => { await update({ reset: false }); editId = null; }} class="grid gap-2 rounded bg-gray-50 p-3 sm:grid-cols-4">
								<input type="hidden" name="id" value={deck.id} />
								<input name="name" required value={deck.name} class="rounded border border-gray-300 px-3 py-2 sm:col-span-2" />
								<input name="language_from" required value={deck.language_from} class="rounded border border-gray-300 px-3 py-2" />
								<input name="language_to" required value={deck.language_to} class="rounded border border-gray-300 px-3 py-2" />
								<div class="sm:col-span-4">
